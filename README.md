...
and auto-generates project-grouped weekly reports. Treats Apple Calendar/Reminders as the
source of truth; an optional launchd "loop" runs a daily sync and a Friday report unattended.
...
> Note: the skill is Chinese-language-oriented (tags 工作/生活, Chinese weekly reports).
...
- "明天之前把 A 项目股东协议改完发给外部律师"
- "下周三下午 2-4 点和管理层开董事会，议题是 B 项目"
- "把上次那个尽调清单的截止时间改到周五"
- "生成本周周报"
...
v1.1-dist · 单机版 · Apple 真源架构（sync）+ 可选 launchd loop。不含跨机协同。

## 反馈

bug 与改进建议请发邮件或在原仓库提 issue。
