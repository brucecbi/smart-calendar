---
name: smart-calendar
description: 基于 macOS / iOS 苹果生态的智能日程与待办事项管理 skill。将非结构化输入（口语描述、聊天截图、邮件截图）自动解析为结构化任务，经用户确认后写入 Apple 日历 / 提醒事项，支持标签管理（工作 / 生活 / 自定义）、时间冲突检测、后续更新删除，以及按项目制自动生成工作周报（项目 - 进展 - 待办三级结构）。语言风格定位为法务 / 投行级正式书面语，也适用于其他需要专业书面表达的知识工作场景。单机版：每台 Mac 独立运行，数据本地存储，不做多端同步。
...
用户在以下场景触发：
1. 输入待办事项的口语描述（如"明天之前把 A 项目股东协议改完发给外部律师"）
...
2. 从输入中提取以下结构化字段：
   - **任务标题**（核心描述）
   - **任务类型**：文件审阅 / 反馈 / 尽职调查 / 资本市场文书 / 会议 / 其他
...
4. **主标签**：基于任务内容判断「工作」or「生活」。判断规则：
   - 涉及业务项目名、专业术语（协议、审阅、尽调、董事会等）、工作相关人员（律师、外部顾问、管理层）→ **工作**
...
8. 用户确认后生成正式用语（专业书面语）：
   ...
   - 示例："明天把 A 项目协议改完" → "A 项目股东协议修订及定稿 —— 根据内部审阅意见完成协议文本修订并发送外部律师确认"
...
11. 根据任务特征智能路由：
    ...
    - **仅日历事件**：有明确起止时间的会议 / 约会（如"周三下午 2-4 点董事会"）
...
```json
{
  ...
  "parsed": {
    "title": "提取标题",
    "task_type": "文件审阅|反馈|尽职调查|资本市场文书|会议|其他",
    "project": "项目名",
